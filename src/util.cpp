...
xcb_window_t util::get_focus(xcb_connection_t *c){
	xcb_get_input_focus_reply_t *r = xcb_get_input_focus_reply(c,xcb_get_input_focus(c),NULL);
	if (!r){
		util::annoy(error,"no focused window found");
		return XCB_NONE;
	}
	xcb_window_t focused_win = r->focus;
	free(r);
...
void util::list_windows(xcb_connection_t *c){
	...
	xcb_query_tree_reply_t *reply = xcb_query_tree_reply(c,xcb_query_tree(c,root),NULL);
	if (!reply) {
		util::annoy(error,"failed to get tree");
		return;
	}
	xcb_window_t *root_children = xcb_query_tree_children(reply);
