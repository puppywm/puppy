...
impl base::ResolveWireError for Error {
    unsafe fn resolve_wire_error(first_error: u8, raw: *mut xcb_generic_error_t) -> Self {
        ...
        match error_code - first_error {
            1 => Error::Request(RequestError::from_raw(raw)),
            2 => Error::Value(ValueError::from_raw(raw)),
            3 => Error::Window(WindowError::from_raw(raw)),
            4 => Error::Pixmap(PixmapError::from_raw(raw)),
            5 => Error::Atom(AtomError::from_raw(raw)),
            6 => Error::Cursor(CursorError::from_raw(raw)),
            7 => Error::Font(FontError::from_raw(raw)),
            8 => Error::Match(MatchError::from_raw(raw)),
            9 => Error::Drawable(DrawableError::from_raw(raw)),
            10 => Error::Access(AccessError::from_raw(raw)),
            11 => Error::Alloc(AllocError::from_raw(raw)),
            12 => Error::Colormap(ColormapError::from_raw(raw)),
            13 => Error::GContext(GContextError::from_raw(raw)),
            14 => Error::IdChoice(IdChoiceError::from_raw(raw)),
            15 => Error::Name(NameError::from_raw(raw)),
            16 => Error::Length(LengthError::from_raw(raw)),
            17 => Error::Implementation(ImplementationError::from_raw(raw)),
            _ => unreachable!(
                "Could not resolve xproto Error with error_code {} and first_error {}",
...
impl base::ResolveWireEvent for Event {
    unsafe fn resolve_wire_event(first_event: u8, raw: *mut xcb_generic_event_t) -> std::option::Option<Self> {
        ...
        match response_type - first_event {
            2 => Some(Event::KeyPress(KeyPressEvent::from_raw(raw))),
            3 => Some(Event::KeyRelease(KeyReleaseEvent::from_raw(raw))),
            4 => Some(Event::ButtonPress(ButtonPressEvent::from_raw(raw))),
            5 => Some(Event::ButtonRelease(ButtonReleaseEvent::from_raw(raw))),
            6 => Some(Event::MotionNotify(MotionNotifyEvent::from_raw(raw))),
            7 => Some(Event::EnterNotify(EnterNotifyEvent::from_raw(raw))),
            8 => Some(Event::LeaveNotify(LeaveNotifyEvent::from_raw(raw))),
            9 => Some(Event::FocusIn(FocusInEvent::from_raw(raw))),
            10 => Some(Event::FocusOut(FocusOutEvent::from_raw(raw))),
            11 => Some(Event::KeymapNotify(KeymapNotifyEvent::from_raw(raw))),
            12 => Some(Event::Expose(ExposeEvent::from_raw(raw))),
            13 => Some(Event::GraphicsExposure(GraphicsExposureEvent::from_raw(raw))),
            14 => Some(Event::NoExposure(NoExposureEvent::from_raw(raw))),
            15 => Some(Event::VisibilityNotify(VisibilityNotifyEvent::from_raw(raw))),
            16 => Some(Event::CreateNotify(CreateNotifyEvent::from_raw(raw))),
            17 => Some(Event::DestroyNotify(DestroyNotifyEvent::from_raw(raw))),
            18 => Some(Event::UnmapNotify(UnmapNotifyEvent::from_raw(raw))),
            19 => Some(Event::MapNotify(MapNotifyEvent::from_raw(raw))),
            20 => Some(Event::MapRequest(MapRequestEvent::from_raw(raw))),
            21 => Some(Event::ReparentNotify(ReparentNotifyEvent::from_raw(raw))),
            22 => Some(Event::ConfigureNotify(ConfigureNotifyEvent::from_raw(raw))),
            23 => Some(Event::ConfigureRequest(ConfigureRequestEvent::from_raw(raw))),
            24 => Some(Event::GravityNotify(GravityNotifyEvent::from_raw(raw))),
            25 => Some(Event::ResizeRequest(ResizeRequestEvent::from_raw(raw))),
            26 => Some(Event::CirculateNotify(CirculateNotifyEvent::from_raw(raw))),
            27 => Some(Event::CirculateRequest(CirculateRequestEvent::from_raw(raw))),
            28 => Some(Event::PropertyNotify(PropertyNotifyEvent::from_raw(raw))),
            29 => Some(Event::SelectionClear(SelectionClearEvent::from_raw(raw))),
            30 => Some(Event::SelectionRequest(SelectionRequestEvent::from_raw(raw))),
            31 => Some(Event::SelectionNotify(SelectionNotifyEvent::from_raw(raw))),
            32 => Some(Event::ColormapNotify(ColormapNotifyEvent::from_raw(raw))),
            33 => Some(Event::ClientMessage(ClientMessageEvent::from_raw(raw))),
            34 => Some(Event::MappingNotify(MappingNotifyEvent::from_raw(raw))),
            _ => None,
        }
    }
}
